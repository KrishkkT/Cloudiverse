...
// 🔥 Trust proxy for correct req.protocol/req.get('host') in production
...
pool.query('SELECT NOW()', async (err, res) => {
  if (err) {
    ...
    const migrationQuery = `
      ...
      -- Ensure workspace_id and source_type exists if table was created previously without it
      -- And ensure the constraint is ON DELETE CASCADE
      DO $$
      BEGIN
          IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deployments' AND column_name='workspace_id') THEN
              ALTER TABLE deployments ADD COLUMN workspace_id INTEGER REFERENCES workspaces(id) ON DELETE CASCADE;
          ELSE
              -- If column exists, we need to drop and recreate the constraint to ensure ON DELETE CASCADE
              ALTER TABLE deployments DROP CONSTRAINT IF EXISTS deployments_workspace_id_fkey;
              ALTER TABLE deployments ADD CONSTRAINT deployments_workspace_id_fkey 
                  FOREIGN KEY (workspace_id) REFERENCES workspaces(id) ON DELETE CASCADE;
          END IF;
      END $$;

      ALTER TABLE deployments ADD COLUMN IF NOT EXISTS source_type VARCHAR(20) DEFAULT 'github';
      ALTER TABLE deployments ADD COLUMN IF NOT EXISTS version INTEGER DEFAULT 1;
      ALTER TABLE deployments ADD COLUMN IF NOT EXISTS storage_path TEXT;
      ...
      BEGIN
          ...
          -- Add project_id to workspaces if missing, ensuring ON DELETE CASCADE
          ...
          ELSE
              -- Ensure existing constraint is ON DELETE CASCADE
          ...
          -- REPAIR DEPLOYMENTS TABLE
          IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='deployments') THEN
              IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deployments' AND column_name='workspace_id') THEN
                  ALTER TABLE deployments ADD COLUMN workspace_id INTEGER REFERENCES workspaces(id);
              END IF;
              IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deployments' AND column_name='source_type') THEN
                  ALTER TABLE deployments ADD COLUMN source_type VARCHAR(20) DEFAULT 'github';
              END IF;
              IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deployments' AND column_name='status') THEN
                  ALTER TABLE deployments ADD COLUMN status VARCHAR(20) DEFAULT 'pending';
              END IF;
              IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deployments' AND column_name='url') THEN
                  ALTER TABLE deployments ADD COLUMN url TEXT;
              END IF;
              IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deployments' AND column_name='commit_hash') THEN
                  ALTER TABLE deployments ADD COLUMN commit_hash VARCHAR(100);
              END IF;
              IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deployments' AND column_name='image_tag') THEN
                  ALTER TABLE deployments ADD COLUMN image_tag VARCHAR(100);
              END IF;
              IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deployments' AND column_name='logs') THEN
                  ALTER TABLE deployments ADD COLUMN logs JSONB DEFAULT '[]';
              END IF;
          END IF;
...
app.use('/api/deploy', require('./routes/deploy')); // New Deployment Route (Activated)
