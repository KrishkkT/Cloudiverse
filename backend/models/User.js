...
class User {
  static async create(userData) {
    ...
    const result = await pool.resilientQuery(query, values);
    return result.rows[0];
  }

  static async findByGoogleId(googleId) {
    const query = 'SELECT * FROM users WHERE google_id = $1';
    const result = await pool.resilientQuery(query, [googleId]);
    return result.rows[0];
  }
  ...
  static async updateGoogleInfo(id, googleId, avatarUrl) {
    const query = `
      ...
      `;
    const result = await pool.resilientQuery(query, [googleId, avatarUrl, id]);
    return result.rows[0];
  }

  static async findByEmail(email) {
    const query = 'SELECT * FROM users WHERE email = $1';
    const result = await pool.resilientQuery(query, [email]);
    return result.rows[0];
  }

  static async findById(id) {
    const query = 'SELECT id, name, email, company, created_at FROM users WHERE id = $1';
    const result = await pool.resilientQuery(query, [id]);
    return result.rows[0];
  }
  ...
  static async delete(id) {
    ...
    await pool.resilientQuery('DELETE FROM workspaces WHERE user_id = $1', [id]);
  ...
  static async updateCloudCredentials(userId, provider, credentials) {
    ...
    `;
    const result = await pool.resilientQuery(query, [JSON.stringify(credentials), userId]);
    return result.rows[0];
  }

  static async getCloudCredentials(userId) {
    const query = 'SELECT cloud_credentials FROM users WHERE id = $1';
    const result = await pool.resilientQuery(query, [userId]);
