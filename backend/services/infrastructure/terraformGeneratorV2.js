...
console.log('[DEBUG] global.regionArgMap initialized');
...
const SERVICE_METADATA = {
  ...
  // Storage
  objectstorage: { args: ['region', 'location'] },  // s3 module DOES accept region
  object_storage: { args: ['region', 'location'] }, // Alias
  blockstorage: { args: [] },
  block_store: { args: [] }, // Alias
  datalake: { args: ['region', 'location'] },
  data_lake: { args: ['region', 'location'] }, // Alias

  // Compute — shared modules (ecs-fargate, lambda) do NOT accept region
  computebatch: { deps: ['networking'], args: ['vpc_id', 'private_subnet_ids'] },
  batch_compute: { deps: ['networking'], args: ['vpc_id', 'private_subnet_ids'] }, // Alias
  computeedge: { args: [] },
  edge_compute: { args: [] }, // Alias
  computeserverless: { args: ['execution_role_arn'] },
  serverless_compute: { args: ['execution_role_arn'] }, // Alias
  ...
  app_container: { deps: ['networking'], args: ['vpc_id', 'private_subnet_ids', 'container_image', 'container_port', 'cpu', 'memory', 'execution_role_arn'] }, // Alias
  appcompute: { deps: ['networking'], args: ['vpc_id', 'private_subnet_ids', 'container_image', 'container_port', 'cpu', 'memory', 'execution_role_arn'] },
  computevm: { deps: ['networking'], args: ['vpc_id', 'private_subnet_ids', 'public_subnet_ids'] },
  vm_compute: { deps: ['networking'], args: ['vpc_id', 'private_subnet_ids', 'public_subnet_ids'] }, // Alias
  ...
  // Network — ALB shared module does NOT accept region
  loadbalancer: { deps: ['networking'], args: ['vpc_id', 'public_subnet_ids'] },
  globalloadbalancer: { args: [] },
  apigateway: { deps: ['networking'], args: ['vpc_id', 'private_subnet_ids'] },
  privatelink: { deps: ['networking'], args: ['vpc_id', 'private_subnet_ids'] },
  private_link: { deps: ['networking'], args: ['vpc_id', 'private_subnet_ids'] }, // Alias
  natgateway: { deps: ['networking'], args: ['vpc_id', 'public_subnet_ids'] },
  nat_gateway: { deps: ['networking'], args: ['vpc_id', 'public_subnet_ids'] }, // Alias
  vpngateway: { deps: ['networking'], args: ['vpc_id'] },
  internetgateway: { deps: ['networking'], args: ['vpc_id'] },
  internet_gateway: { deps: ['networking'], args: ['vpc_id'] }, // Alias
  dns: { args: [] },
  servicediscovery: { deps: ['networking'], args: ['vpc_id'] },
  service_discovery: { deps: ['networking'], args: ['vpc_id'] }, // Alias
  servicemesh: { deps: ['networking'], args: ['vpc_id'] },
  service_mesh: { deps: ['networking'], args: ['vpc_id'] }, // Alias
  vpn: { deps: ['networking'], args: ['vpc_id'] },
  networkfirewall: { deps: ['networking'], args: ['vpc_id', 'public_subnet_ids'] },
  firewall: { deps: ['networking'], args: ['vpc_id', 'public_subnet_ids'] }, // Alias
  securitygroup: { deps: ['networking'], args: ['vpc_id'] },
  security_group: { deps: ['networking'], args: ['vpc_id'] }, // Alias
  egressproxy: { deps: ['networking'], args: ['vpc_id'] },
  egress_proxy: { deps: ['networking'], args: ['vpc_id'] }, // Alias
  transitgateway: { deps: ['networking'], args: ['vpc_id', 'private_subnet_ids'] },
  transit_gateway: { deps: ['networking'], args: ['vpc_id', 'private_subnet_ids'] }, // Alias
  websocketgateway: { deps: ['networking'], args: ['vpc_id'] },
  cdn: { args: ['bucket_domain_name', 'bucket_name', 'bucket_arn', 'region', 'location'] },  // cdn module DOES accept region
...
resource "aws_cloudwatch_metric_alarm" "cpu" {
  ...
  threshold           = 80
    ...
    if (has('logging')) {
      addResource('aws_cloudwatch_log_group', 'app', `
resource "aws_cloudwatch_log_group" "app" {
  name              = "/ecs/\${shortName}-app"
    ...
    services.forEach(service => {
      ...
      if (moduleName === 'cdn') {
        ...
        if (hasObjectStorage) {
          ...
          mainTf += `    bucket_domain_name = ${getModRef('objectstorage')}.bucket_regional_domain_name\n`;
...
async function generateTerraform(canonicalArchitecture, provider, region, projectName, options = {}) {
  ...
  const modules = generateModules(deployableServices, providerLower, region, projectName, pattern);
 ...
 */
function generateModules(services, provider, region, projectName, pattern = 'CUSTOM') {
  const modules = {};
...
resource "aws_cloudfront_origin_access_control" "oac" {
  ...
  # ═══════════════════════════════════════════════════════════════════════════
  # SPA & Pretty URL Support
  # ═══════════════════════════════════════════════════════════════════════════

  # 1. CloudFront Function for URL Rewriting (Clean URLs: /cbam -> /cbam.html)
  resource "aws_cloudfront_function" "url_rewrite" {
    name    = "url-rewrite-\${random_id.cdn_suffix.hex}"
    runtime = "cloudfront-js-1.0"
    comment = "Handles index.html and .html extensions for subpaths"
    publish = true
    code    = <<EOF
function handler(event) {
    var request = event.request;
...
EOF
  ...
  resource "aws_cloudfront_distribution" "cdn" {
    ...
    origin {
      domain_name = var.bucket_domain_name # Passed as regional domain name from main.tf
      origin_id   = "site"
    ...
    # SPA-style fallback: Redirect 403/404 to index.html for client-side routing
