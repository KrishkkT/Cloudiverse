...
class TerraformExecutor {
        ...
        return new Promise((resolve) => {
            ...
            const proc = spawn(TERRAFORM_BIN, [command, ...args], {
                ...
                env: {
                    ...
                    // 🔥 FIX: Force Go net stack to use Go resolver (avoids IPv6 issues on Windows)
                    GODEBUG: envVars.GODEBUG || 'netdns=go',
                },
                shell: true
            });
    ...
    async startRealTerraformExecution(jobId, provider, workspaceId, terraformFiles, connectionData) {
        ...
        try {
            ...
            // 🔥 FIX: Force IPv4 to prevent IPv6 connection failures on Windows
            // Terraform uses Go's net stack which may prefer IPv6 and fail on some networks
            const initEnv = {
                ...envVars,
                GODEBUG: 'netdns=go',
                // Force Go HTTP transport to prefer IPv4
                GOPROXY: 'direct',
            };

            // Set up plugin cache to avoid re-downloading providers on every run
            const pluginCacheDir = path.join(TERRAFORM_WORK_DIR, '_plugin_cache');
            try {
                await fs.mkdir(pluginCacheDir, { recursive: true });
                initEnv.TF_PLUGIN_CACHE_DIR = pluginCacheDir;
                this.addLog(jobId, `Plugin cache enabled at: ${pluginCacheDir}`, 'INFO');
            } catch (cacheErr) {
                // Non-critical
            }

            // 🔥 RESILIENCE FIXES
            initEnv.TF_IN_AUTOMATION = '1';
            initEnv.CHECKPOINT_DISABLE = '1'; // Disable telemetry to avoid extra network calls
            initEnv.TF_REGISTRY_CLIENT_TIMEOUT = '600'; // Increase timeout to 10 minutes

            // Retry init up to 3 times to handle transient network failures
            let initResult = null;
            const maxRetries = 3;
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                // On retry, enable more logging to help diagnose
                if (attempt > 1) {
                    initEnv.TF_LOG = 'INFO';
                }

                initResult = await this.runTerraformCommand(
                    jobId,
                    'init',
                    initArgs,
                    workDir,
                ...
                if (attempt < maxRetries) {
                    this.addLog(jobId, `Init failed (attempt ${attempt}/${maxRetries}). Retrying in 5s...`, 'WARN');
                    // Clear .terraform to retry cleanly
                    try { await fs.rm(path.join(workDir, '.terraform'), { recursive: true, force: true }); } catch (e) {}
                    try { await fs.rm(path.join(workDir, '.terraform.lock.hcl'), { force: true }); } catch (e) {}
