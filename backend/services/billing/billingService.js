...
class BillingService {
    ...
    async getUserBySubId(razorpaySubId) {
        const res = await pool.resilientQuery(
    ...
    async getUserById(userId) {
        const res = await pool.resilientQuery('SELECT email, name FROM users WHERE id = $1', [userId]);
        return res.rows[0];
    }
    ...
    async createSubscriptionIntent(userId) {
        // 1. Check existing
        const existing = await pool.resilientQuery('SELECT * FROM subscriptions WHERE user_id = $1', [userId]);
    ...
    async linkSubscription(userId, razorpaySubId, razorpayPaymentId, razorpaySignature) {
        ...
        await pool.resilientQuery(query, [userId, razorpaySubId, currentEnd]);
