...
function extractExplicitServicesFromText(text) {
  ...
  const isNegated = (str, index) => {
    const prefix = str.substring(Math.max(0, index - 30), index);
    // Handle "without [word1], [word2]...", "no [word1] or [word2]", etc.
    return /\b(no|without|do not need|don't need|exclude|excluding|minus)\s+([\w\s,]+)?$/.test(prefix);
  };
...
class PatternResolver {
  ...
  extractRequirements(intent) {
    try {
      ...
      */
      ...
      // Determine if stateful (only if not already excluded)
      if (!requirements.stateful && !terminalExclusions.includes('database')) {
        if (text.includes('database') || text.includes('store') || text.includes('save') ||
          ...
          requirements.data_stores.length > 0) {
          requirements.stateful = true;
        }
      }
      ...
      // Determine authentication (only if not already excluded)
      if (!requirements.authentication && !terminalExclusions.includes('auth') && !terminalExclusions.includes('authentication')) {
      ...
      // 🆕 NEW: Map decision axes to data stores (only if not excluded)
      if (!terminalExclusions.includes('database')) {
        if (decisionAxes.primary_data_model && decisionAxes.primary_data_model.includes('relational')) {
          if (!requirements.data_stores.includes('relationaldatabase')) {
            requirements.data_stores.push('relationaldatabase');
          }
        }
        if (decisionAxes.primary_data_model && decisionAxes.primary_data_model.includes('nosql')) {
          if (!requirements.data_stores.includes('nosqldatabase')) {
            requirements.data_stores.push('nosqldatabase');
          }
        }
      }
      
      if (decisionAxes.file_storage === true && !terminalExclusions.includes('storage')) {
        if (!requirements.data_stores.includes('objectstorage')) {
          requirements.data_stores.push('objectstorage');
        }
      }
      if (decisionAxes.messaging_queue === true && !terminalExclusions.includes('queue')) {
        if (!requirements.data_stores.includes('messagequeue')) {
          requirements.data_stores.push('messagequeue');
        }
      }

      // Determine data stores (only if not excluded)
      if (!terminalExclusions.includes('database')) {
        if (!requirements.data_stores.includes('relationaldatabase')) {
          if (text.includes('sql') || text.includes('database') || text.includes('relational') ||
            text.includes('mysql') || text.includes('postgres')) {
            // Re-verify negation for safety
            const idx = text.indexOf('database');
            if (idx === -1 || !isNegated(text, idx)) {
              requirements.data_stores.push('relationaldatabase');
              requirements.stateful = true;
            }
          }
      ...
      if (!terminalExclusions.includes('cache') && (text.includes('cache') || text.includes('redis'))) {
        if (!requirements.data_stores.includes('cache')) {
          requirements.data_stores.push('cache');
        }
      }

      if (!terminalExclusions.includes('queue') && (text.includes('queue') || text.includes('message') || text.includes('kafka'))) {
        if (!requirements.data_stores.includes('messagequeue')) {
          requirements.data_stores.push('messagequeue');
        }
      }

      if (!terminalExclusions.includes('storage') && (text.includes('file') || text.includes('storage') || text.includes('s3') || text.includes('images') || text.includes('photos'))) {
        if (!requirements.data_stores.includes('objectstorage')) {
          requirements.data_stores.push('objectstorage');
        }
      }
  ...
  resolveArchitectureV2(preIntentContext, v2Axes) {
    ...
    // Map capabilities to data stores (only if not excluded)
    if (capabilities.has('relational_db') && !requirements.terminal_exclusions.includes('database')) {
      requirements.data_stores.push('relationaldatabase');
    }
    if (capabilities.has('object_storage') && !requirements.terminal_exclusions.includes('storage')) {
      requirements.data_stores.push('objectstorage');
    }
    if (capabilities.has('message_queue') && !requirements.terminal_exclusions.includes('queue')) {
      requirements.data_stores.push('messagequeue');
    }
    ...
    if (explicitServices.data_stores.length > 0) {
      ...
      explicitServices.data_stores.forEach(svc => {
        // Double-check exclusions for text-detected services too
        const isExcluded = requirements.terminal_exclusions.includes(svc) || 
                          (svc === 'relationaldatabase' && requirements.terminal_exclusions.includes('database')) ||
                          (svc === 'objectstorage' && requirements.terminal_exclusions.includes('storage'));
        
        if (!isExcluded && !requirements.data_stores.includes(svc)) {
          requirements.data_stores.push(svc);
        }
      });
    }

    // Merge explicit capabilities from text (only if not excluded)
    if (explicitServices.capabilities.payments && !requirements.terminal_exclusions.includes('payments')) {
      requirements.payments = true;
    }
    if (explicitServices.capabilities.auth && !requirements.terminal_exclusions.includes('auth') && !requirements.terminal_exclusions.includes('authentication')) {
      requirements.authentication = true;
    }
    if (explicitServices.capabilities.cache && !requirements.terminal_exclusions.includes('cache')) {
      requirements.capabilities.cache = true;
    }
    if (explicitServices.capabilities.search && !requirements.terminal_exclusions.includes('search')) {
      requirements.capabilities.search = true;
    }
