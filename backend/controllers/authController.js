    ...
    if (req.body.device_id) {
      ...
      try {
        if (user.device_id !== req.body.device_id) {
          await pool.resilientQuery("UPDATE users SET device_id = $1 WHERE id = $2", [req.body.device_id, user.id]);
        }
      } catch (err) {
...
const forgotPassword = async (req, res) => {
  try {
    ...
    await pool.resilientQuery('DELETE FROM password_resets WHERE email = $1', [email]);
...
const resetPassword = async (req, res) => {
  try {
    ...
    // Update User
    await pool.resilientQuery("UPDATE users SET password = $1 WHERE email = $2", [hashedPassword, email]);
...
const updatePassword = async (req, res) => {
  try {
    ...
    // Update
    await pool.resilientQuery("UPDATE users SET password = $1 WHERE id = $2", [hashedPassword, req.user.id]);
...
const deleteAccount = async (req, res) => {
  try {
    ...
    const activeDeploymentsRes = await pool.resilientQuery(
