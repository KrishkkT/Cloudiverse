...
router.get('/connections/:provider', authMiddleware, async (req, res) => {
    try {
        ...
        const wsRes = await pool.resilientQuery("SELECT state_json FROM workspaces WHERE id = $1", [workspace_id]);
...
router.post('/:provider/connect', authMiddleware, async (req, res) => {
    try {
        ...
        if (provider === 'aws') {
            ...
            // 🔥 DYNAMIC URL: Ensure the download link matches the environment (localhost vs cloudiverse.app)
            const host = req.get('host');
            const protocol = req.protocol;
            const baseUrl = `${protocol}://${host}`;
            const downloadUrl = `${baseUrl}/api/cloud/aws/template/download/${workspace_id}`;
