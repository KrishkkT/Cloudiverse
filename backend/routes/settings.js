...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        const projectCountRes = await pool.resilientQuery(
...
router.get('/:workspaceId/activities', authMiddleware, async (req, res) => {
    try {
        const { workspaceId } = req.params;
        const activities = await auditService.getActivities(workspaceId);
        res.json(activities);
    } catch (err) {
